...
function accumulatorFor(pending) {
...
export function newQuestionToken() {
    return randomUUID();
}
/** 渲染问题文本与内联键盘。 */
export function renderQuestions(questions, token) {
...
/** 回填渲染时用到的 meta（供解析）。 */
export function questionMetaOf(questions, token) {
    return {
        token,
...
export function completeQuestion(pending) {
    const meta = pending.meta;
    const acc = accumulatorFor(pending);
    if (meta === undefined || meta.items.length === 0)
        return undefined;
    const answers = [];
...
export function answerFromCallback(pending, data) {
    const meta = pending.meta;
    if (meta === undefined)
        return false;
    ...
    if (q === undefined || q.multiSelect || q.options[li] === undefined)
        return false;
 ...
 */
export function answerFromText(pending, text) {
    const meta = pending.meta;
    if (meta === undefined || meta.items.length === 0)
