...
import { StateStore, migrateLegacyState, botDataDir } from './core/state-store.js';
import { assertSessionOwnership, resolveBotScopes, joinDefaultDataDir, routeKey } from './core/bot-scope.js';
...
export function apply(ctx, config) {
    ...
    for (const [dir, ids] of dataDirBots) {
        ...
        if (report.backup !== undefined)
            logger.warn(`旧共享状态已备份: ${report.backup}`);
    }
     ...
     */
    const readSessionModelFromCache = (sessionId) => {
        const pick = (value) => {
    ...
    const sweepTimer = setTimeout(() => {
        for (const scope of scopes) {
            ...
            for (const chat of Object.values(store.allChats())) {
                if (chat.sessionId === undefined || chat.sessionId === '' || chat.cwd === undefined || chat.cwd === '')
                    continue;
    ...
    for (const scope of scopes) {
        logger.warn(`bot "${scope.botId}" 授权: allowAllUsers=${scope.allowAllUsers}`
    ...
    const seedBinding = (chatId, botId, sessionId) => {
    ...
    const hostSessionRoster = async () => {
        ...
        const isSubagent = (s) => s?.origin === 'subagent';
        // Archived sessions are hidden by the GUI too; mirror that here.
        const archived = (() => {
            try {
                const reg = ctx.get?.('workspaceRegistry');
                return new Set(reg?.archivedSessionIds ?? []);
            }
            catch {
                return new Set();
            }
        ...
        const merge = (s) => {
            if (s === undefined || !s.id || isSubagent(s) || archived.has(s.id))
                return;
        ...
        try {
            ...
            const projNumber = (session, key, field) => {
                const state = projections?.stateOf?.(session, key);
            ...
            for (const s of svc?.list?.() ?? []) {
                ...
                const meta = projections?.stateOf?.(s, 'sessionListMetadata');
                merge({
                    id,
                    cwd: header?.cwd,
        ...
        try {
            ...
            for (const name of readdirSync(sdir)) {
                ...
                let parsed;
                try {
                    parsed = JSON.parse(readFileSync(join(sdir, name), 'utf8'));
                }
                catch {
                    continue;
                }
        ...
        try {
            ...
            if (gw?.invoke !== undefined) {
                for (const ns of ['session', 'sessions']) {
     ...
     * 存在插件级状态里；创建完成或取消时清空。
     */
    const newSessionDrafts = new Map();
    ...
    const menuCtxFor = (chatId, botId) => {
        ...
        return {
            ...
            listModels: async () => {
                ...
                const ids = providers.length > 0 ? providers.map(p => p.id ?? p.name).filter((x) => Boolean(x)) : [scope.provider];
                for (const id of ids) {
                    try {
            ...
            listWorkspaces: async () => {
                const set = new Set([defaultCwd, ...scope.workspaceRoots]);
                // Host-wide workspaces (GUI registry, other bots' sessions) are only
                // revealed when this bot explicitly opted in.
                if (!scope.allowHostSessions)
                    return [...set];
                try {
                    const gw = ctx.get?.('typertGateway');
                    if (gw?.invoke !== undefined) {
                        for (const ns of ['workspace', 'workspaces']) {
                            const value = await gw.invoke({ namespace: ns, method: 'list', args: {} });
                            const items = value?.items ?? [];
                            for (const it of items)
            ...
            switchSession: async (sessionId, cwd) => {
                if (!scope.allowHostSessions && !sessions.ownsSession(botId, sessionId)) {
                    throw new Error(`会话 ${sessionId} 不属于 bot "${botId}";严格隔离下不能附加到其他 Bot/GUI 的会话。`
            ...
            renameSession: async (sessionId, title) => {
                ...
                try {
                    ...
                    if (handleToDispose !== undefined) {
                        await handleToDispose.dispose().catch(() => { });
                    }
                }
            },
            /**
    ...
    const consumeRename = async (text, chatId, botId) => {
    ...
    const manager = new BotManager({
        ...
        respond: {
            onCallback,
            onText: (text, chatId, botId) => consumeRename(text, chatId, botId).then(consumed => consumed || onText(text, chatId, botId)),
        },
        logger,
    });
    ...
    const announceRestart = async (attempt = 0) => {
        if (noticeKind === 'none') {
        ...
        for (const scope of scopes) {
            ...
            for (const [key, chat] of Object.entries(store.allChats())) {
                if (chat.sessionId === '' || chat.sessionId === undefined)
                    continue;
    ...
    const flushTimer = setInterval(() => {
        for (const [id, runtime] of manager.all) {
            try {
                ...
                if (offset !== undefined)
                    stores.get(id)?.setOffset(id, offset);
                stores.get(id)?.flush();
            }
            catch (error) {
