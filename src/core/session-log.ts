...
/** One complete frame's byte range inside the concatenated file. */
 ...
 */
export function indexSessionLogs(sessionsRoot: string): Map<string, { path: string; mtimeMs: number }> {
  const out = new Map<string, { path: string; mtimeMs: number }>()
  let workspaces: string[] = []
  try { workspaces = readdirSync(sessionsRoot) } catch { return out }
  for (const ws of workspaces) {
    ...
    for (const dir of dirs) {
      ...
      let mtimeMs = 0
      try { mtimeMs = statSync(file).mtimeMs } catch { /* keep 0 */ }
...
export function decodeSessionId(encoded: string): string {
  return encoded.replace(/~([0-9A-Fa-f]{4})/g, (_m, hex: string) => String.fromCodePoint(Number.parseInt(hex, 16)))
}
