...
import { assertSessionOwnership, resolveBotScopes, routeKey } from './core/bot-scope.js';
...
export function apply(ctx, config) {
    ...
    const hostSessionRoster = async () => {
        ...
        const push = (s) => {
            if (s === undefined || !s.id || seen.has(s.id) || isSubagent(s))
                return;
            seen.add(s.id);
            out.push({
                id: s.id,
        ...
        for (const scope of scopes) {
            ...
            for (const [key, chat] of Object.entries(store.allChats())) {
                ...
                if (!Number.isFinite(chatId))
                    continue;
                void delivery.sendFinal(chatId, '✅ DSH 已重新上线,会话已恢复。').catch((error) => {
