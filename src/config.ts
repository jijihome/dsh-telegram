...
export const Config: Schema<TelegramConfig> = Schema.object({
  ...
  restartNotice: Schema.union(['always', 'marked', 'off']).default('always'),
  restartNoticeMaxGapMs: Schema.number().default(0),
  workspaceRoots: Schema.array(Schema.string()),
  dataDir: Schema.string(),
  proxy: Schema.string(),
  ...
  pendingQuestionTimeoutSec: Schema.number().default(300),
  approvalDecisionWords: Schema.array(Schema.string()).default(['批准', '同意', 'yes']),
  rejectDecisionWords: Schema.array(Schema.string()).default(['拒绝', '不同意', 'no']),
})
