...
async function doMenuSessions(ctx) {
    ...
    // Mark the session this chat is currently on — config binding, live binding, or
    // the id persisted for it — so the list still shows the active conversation
    // right after a DSH restart (when no binding has been re-registered yet).
