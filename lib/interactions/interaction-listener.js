    ...
    onHostEvent('user-questions/request', async (request0, next) => {
        ...
        const answer = await new Promise(resolveWait => {
            ...
            try {
                ...
                void delivery.sendMenu(target.chatId, text, keyboard).then(() => logger?.warn(`[交互] SeamA sendMenu ok`), (e) => logger?.warn(`[交互] SeamA sendMenu FAILED: ${String(e)}`)).catch(() => {
    ...
    onHostEvent('approval/request', async (request0, next) => {
        ...
        const outcome = await new Promise(resolveWait => {
            ...
            try {
                ...
                void delivery.sendMenu(target.chatId, text, keyboard).then(() => logger?.warn(`[交互] SeamB sendMenu ok`), (e) => logger?.warn(`[交互] SeamB sendMenu FAILED: ${String(e)}`)).catch(() => {
                    store.finish(pending, undefined);
                });
            }
            catch (error) {
