...
export class SessionManager {
    ...
    cancel(chatId, botId) {
        const key = routeKey(botId, chatId);
        // A 会话-menu pick lives in `bound` (driven through `boundFollowup`), NOT in
        // `bindings`, so looking only at bindings made /stop answer「当前没有运行中的
        // 回合」while that agent was actively generating. Resolve the agent the chat is
        // ACTUALLY driving, in the same order routing uses (see bot-manager
        // handleUpdate): bound session → own live binding → active session.
        const boundId = this.getBound(chatId, botId)?.sessionId;
        let agent = boundId !== undefined ? this.factory.getLive(boundId) : undefined;
        if (boundId === undefined) {
            agent = this.bindings.get(key)?.handle.agent;
            if (agent === undefined) {
                const activeId = this.activeSessionId(chatId, botId);
                agent = activeId !== undefined ? this.factory.getLive(activeId) : undefined;
            }
        }
        if (agent === undefined)
            return false;
        try {
            // Deliberately NOT gated on `agent.status === 'running'`: a turn parked on
            // user input (or otherwise quiescent mid-turn) reports `idle`, and refusing
            // there would recreate the very false negative this fixes. `cancel` is a
            // documented no-op when no activity is active and never arms later work, so
            // cancelling a found agent is always safe.
            agent.cancel({ kind: 'user' });
        }
        catch (error) {
            this.logger?.warn(`[tg] /stop cancel failed for ${agent.session.id}: ${messageOf(error)}`);
            return false;
        }
        this.logger?.warn(`[tg] /stop cancel ${agent.session.id} (status=${agent.status})`);
        return true;
    }
    /**
