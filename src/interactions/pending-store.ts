...
export interface QuestionMeta {
  ...
  items: { id: string; options: string[]; multiSelect: boolean }[]
}
...
export class PendingStore {
  ...
  private readonly logger: PendingStoreOptions['logger']
  ...
  create(kind: PendingKind, chatId: number, botId: string, timeoutMs: number): Pending | undefined {
    ...
    const pending: Pending = { kind, chatId, botId, resolve: () => {} }
  ...
  finish(pending: Pending, value: unknown): void {
    ...
    try { pending.resolve(value) } catch { /* 忽略 */ }
  }
