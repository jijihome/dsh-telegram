...
function commandStart(ctx) {
    return {
        handled: true,
        reply: `🤖 dsh-telegram 已就绪。\n\n支持命令:${COMMANDS.join(' ')}\n直接发消息即可让 agent 处理。`,
    };
}
function commandHelp(ctx) {
    return {
        handled: true,
        reply: [
            '📖 命令说明:',
...
async function commandNew(ctx) {
    try {
        const binding = await ctx.sessions.rotate(ctx.chatId, ctx.botId);
        return { handled: true, reply: `✅ 已开启新会话(session=${binding.sessionId})` };
    }
    catch (error) {
...
async function commandWorkspace(text, ctx) {
    ...
    const arg = args.trim();
    if (arg === '') {
        const lines = [`📁 当前工作目录:${current}`, '', '可选根目录:'];
    ...
    if (binding !== undefined) {
        // Switch the *next* created session's cwd by recording it in the store;
        // the live agent keeps its current cwd until /new.
        ctx.store.setChat(`${ctx.botId}:${ctx.chatId}`, {
