...
export class StateStore {
    ...
    assertKey(key) {
        ...
        if (!key.startsWith(`${this.botId}:`)) {
            throw new Error(`dsh-telegram: 状态键跨 Bot 越界 — store(${this.botId}) 收到键 "${key}";`
 ...
 */
export function migrateLegacyState(dataDir, botIds, logger) {
    const legacyFile = join(dataDir, 'state.json');
    const report = { migrated: [], orphans: [] };
    ...
    const perBotChats = new Map();
    const perBotOffsets = new Map();
    ...
    if (ambiguous.length > 0) {
        throw new Error(`dsh-telegram: 旧状态文件含裸 chatId 键,但配置了 ${botIds.length} 个 Bot,无法判断归属:\n- `
            + ambiguous.join('\n- ')
    ...
    for (const [botId, offset] of Object.entries(offsets)) {
        if (botIds.includes(botId))
            perBotOffsets.set(botId, offset);
    }
    for (const key of report.orphans) {
        if (COMPOSITE_KEY.test(key))
            logger?.warn(`[tg] 迁移跳过未知 Bot 的旧状态键: ${key}`);
    }
