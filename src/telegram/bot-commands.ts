...
import type { TelegramClientLike, BotCommand, BotCommandScopeDefault, MenuButton } from './api.js'
...
export const BOT_COMMANDS: BotCommand[] = [
  ...
  { command: 'clear', description: '清除当前会话' },
...
export const DEFAULT_MENU_BUTTON: MenuButton = { type: 'commands' }
...
export async function registerBotCommands(
  ...
  const scope: BotCommandScopeDefault = { type: 'default' }
...
export async function registerMenuButton(
  client: TelegramClientLike,
  button: MenuButton = DEFAULT_MENU_BUTTON,
): Promise<void> {
 ...
 * @param client - the bot's Telegram API client.
 */
export async function registerBotUi(client: TelegramClientLike): Promise<void> {
