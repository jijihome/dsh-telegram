...
import { agentSessionId, type ApprovalOutcome, type AskUserQuestionAnswer, type ApprovalRequest, type UserQuestionsRequest } from './types.js'
...
function resolveChatTarget(
  ...
  if (active !== undefined) return { chatId: active.chatId, botId: active.botId }
  const bound = sessions.byBoundSessionIds(sid)[0]
  if (bound !== undefined) return { chatId: bound.chatId, botId: bound.botId }
  return undefined
}

/** 注册两个瀑布监听，返回入站应答钩子。 */
export function registerInteractions(options: InteractionOptions): InteractionRespond {
  ...
  const onHostEvent = (name: string, listener: (...args: any[]) => any) => (ctx.on as any)(name, listener)

  // ---- Seam A：user-questions/request ----
  onHostEvent('user-questions/request', async (request0: unknown, next: () => Promise<AskUserQuestionAnswer>) => {
    const request = request0 as UserQuestionsRequest
    const target = resolveChatTarget(sessions, request.agent)
    if (target === undefined) return next()
    const delivery = deliveries.get(target.botId)
    if (delivery === undefined) return next()
  ...
  onHostEvent('approval/request', async (request0: unknown, next: () => Promise<ApprovalOutcome>) => {
    const request = request0 as ApprovalRequest
    const target = resolveChatTarget(sessions, request.agent)
    if (target === undefined) return next()
    const delivery = deliveries.get(target.botId)
    if (delivery === undefined) return next()
  ...
  return {
    ...
    async onCallback(data: string, chatId: number, botId: string): Promise<boolean> {
      ...
      if (question !== undefined && answerFromCallback(question, data)) {
        ...
        if (value !== undefined) store.finish(question, value)
        return true
      }
    ...
    async onText(text: string, chatId: number, botId: string): Promise<boolean> {
      const approval = store.active('approval', chatId, botId)
      if (approval !== undefined) {
      ...
      if (question !== undefined) {
        ...
        if (r.consumed) {
          if (r.value !== undefined) store.finish(question, r.value)
          return true
        }
      }
      return false
